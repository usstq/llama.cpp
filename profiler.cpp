...
std::vector<Profiler> g_profilers;


extern "C" void * startProfile(int ithr, const char * cat, const char * name) {
    auto& inst = g_profilers[ithr];
    if (!inst.enabled) {
        return nullptr;
    }
    inst.all_data.emplace_back(cat, name);
    void * pv = reinterpret_cast<void*>(&inst.all_data.back());
    return pv;
}

extern "C" void stopProfile(void * pv) {
    if (pv) {
        auto* pd = reinterpret_cast<detail::ProfileData*>(pv);
        detail::ProfileData::record_end(pd);
    }
}

void createProfilers(int i) {
    g_profilers.resize(i);
}
